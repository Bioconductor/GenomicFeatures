...
#' \link{TxDb} object from transcript annotations available on a BioMart
#' database.
#' 
#' Note that \code{makeTxDbFromBiomart} is being phased out in favor of
#' \code{\link{makeTxDbFromEnsembl}}.
#' 
#' \code{makeTxDbFromBiomart} is a convenience function that feeds data from a
#' BioMart database to the lower level \code{\link{makeTxDb}} function.  See
#' \code{?\link{makeTxDbFromUCSC}} for a similar function that feeds data from
#' the UCSC source.
#' 
#' Here is a list of datasets known to be compatible with
#' \code{makeTxDbFromBiomart} (list updated on September 18, 2017): \enumerate{
#' \item All the datasets in the main Ensembl database.  Get the list with:
#' \preformatted{ mart <- biomaRt::useEnsembl(biomart="ENSEMBL_MART_ENSEMBL",
#' host="https://www.ensembl.org") biomaRt::listDatasets(mart)}
#' 
#' \item All the datasets in the Ensembl Fungi database.  Get the list with:
#' \preformatted{ mart <- biomaRt::useEnsemblGenomes(biomart="fungi_mart")
#' biomaRt::listDatasets(mart)}
#' 
#' \item All the datasets in the Ensembl Metazoa database.  Get the list with:
#' \preformatted{ mart <- biomaRt::useEnsemblGenomes(biomart="metazoa_mart")
#' biomaRt::listDatasets(mart)}
#' 
#' \item All the datasets in the Ensembl Plants database.  Get the list with:
#' \preformatted{ mart <- biomaRt::useEnsemblGenomes(biomart="plants_mart")
#' biomaRt::listDatasets(mart)}
#' 
#' \item All the datasets in the Ensembl Protists database.  Get the list with:
#' \preformatted{ mart <- biomaRt::useEnsemblGenomes(biomart="protists_mart")
#' biomaRt::listDatasets(mart)}
#' 
#' \item All the datasets in the Gramene Mart.  Get the list with:
#' \preformatted{ mart <- biomaRt::useEnsembl(biomart="ENSEMBL_MART_PLANT",
#' host="https://ensembl.gramene.org") biomaRt::listDatasets(mart)} }
#' 
...
#' named list. An example is \code{filter=list(source="entrez")}
...
#' @param host The host URL of the BioMart. Defaults to www.ensembl.org.
...
#' on the host input.
#' @param taxonomyId By default this value is NA and the dataset selected will
#' be used to look up the correct value for this.  But you can use this
#' argument to override that and supply your own taxId here (which will be
#' independently checked to make sure its a real taxonomy id).  Normally you
...
#' @return A \link{TxDb} object for \code{makeTxDbFromBiomart}.
#' 
...
#' \code{\link{makeTxDbFromEnsembl}} for making a \link{TxDb} object from other
#' online resources.
#' 
...
#' \code{\link[biomaRt]{useEnsembl}}, \code{\link[biomaRt]{listDatasets}}, and
#' \code{\link[biomaRt]{listFilters}} functions in the \pkg{biomaRt} package.
#' 
