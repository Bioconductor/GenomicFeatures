...
#' Making a FeatureDb object from annotations available at the UCSC Genome
#' Browser
#' 
...
#' 
#' The \code{makeFeatureDbFromUCSC} function allows the user to make a
#' \link{FeatureDb} object from simple annotation tracks at UCSC.  The tracks
#' in question must (at a minimum) have a start, end and a chromosome
#' affiliation in order to be made into a \link{FeatureDb}.  This function
#' requires a precise declaration of its first three arguments to indicate
#' which genome, track and table wish to be imported.  There are discovery
#' functions provided to make this process go smoothly.
#' 
#' \code{makeFeatureDbFromUCSC} is a convenience function that builds a tiny
#' database from one of the UCSC track tables.
#' \code{supportedUCSCFeatureDbTracks} a convenience function that returns
#' potential track names that could be used to make FeatureDb objects
#' \code{supportedUCSCFeatureDbTables} a convenience function that returns
#' potential table names for FeatureDb objects (table names go with a track
#' name) \code{UCSCFeatureDbTableSchema} A convenience function that creates a
#' named vector of types for all the fields that can potentially be supported
#' for a given track.  By default, this will be called on your specified
#' tablename to include all of the fields in a track.
#' 
#' % \code{\link{makeFeatureDb}} % function.  % See
#' \code{?\link{makeFeatureDbFromBiomart}} for a similar function % that feeds
#' data from a BioMart database.
#' 
...
#' a particular genome
#' @param tablename name of the UCSC table containing the annotations to
#' retrieve. Use the \code{supportedUCSCFeatureDbTables} utility function to
#' get the list of supported tables for a track.
...
#' be used to look up the correct value for this.  But you can use this
#' argument to override that and supply your own valid taxId here.
#' @return A \link{FeatureDb} object for \code{makeFeatureDbFromUCSC}.  Or in
#' the case of \code{supportedUCSCFeatureDbTracks} and
#' \code{UCSCFeatureDbTableSchema} a named character vector
#' @author M. Carlson
#' @seealso \code{\link[rtracklayer]{ucscGenomes}}, %
#' \code{\link{makeFeatureDbFromBiomart}}, % \code{\link{makeFeatureDb}}
#' @examples
#' 
