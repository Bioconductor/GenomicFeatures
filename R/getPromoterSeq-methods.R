...
#' (a\link{GRanges} or \link{GRangesList} object) from a
#' \link[BSgenome]{BSgenome} object or an \link[Rsamtools]{FaFile}.
#'
#' \code{getPromoterSeq} is an overloaded method dispatching on query, which is
#' either a GRanges or a GRangesList.  It is a wrapper for the \code{promoters}
#' and \code{getSeq} functions. The purpose is to allow sequence extraction
#' from either a \link[BSgenome]{BSgenome} or \link[Rsamtools]{FaFile}.
#'
...
#' @param query A \link[GenomicRanges]{GRanges} or
#' \link[GenomicRanges]{GRangesList} object containing genes grouped by
#' transcript.
#' @param subject A \link[BSgenome]{BSgenome} object or a
#' \link[Rsamtools]{FaFile} from which the sequences will be taken.
#' @param upstream The number of DNA bases to include upstream of the TSS
#' (transcription start site)
#' @param downstream The number of DNA bases to include downstream of the TSS
#' (transcription start site)
#' @param \dots Additional arguments
#' @return A \link[Biostrings]{DNAStringSet} or
#' \link[Biostrings]{DNAStringSetList} instance corresponding to the GRanges or
#' GRangesList supplied in the query.
#' @author Paul Shannon
#' @seealso \link[IRanges]{intra-range-methods} ## promoters method for
#' IntegerRanges objects \link[GenomicRanges]{intra-range-methods} ## promoters
#' method for GenomicRanges objects \link[Biostrings]{getSeq}
