...
\Rmethod{seqlevels} method.  For example:
...
methods...  If you need to reset back to the original seqlevels, use
the \Rmethod{restoreSeqlevels} method.
<<seqlevels3>>=
...
\begin{Exercise}
Use \Rmethod{seqlevels} to set only chromsome 15 to be active.  BTW,
...
written supporting \Rmethod{columns}, \Rmethod{keytypes}, \Rmethod{keys}
