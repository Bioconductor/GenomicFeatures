...
each other, not just their location.  For example, it might be of
...
%% FIXME: list the other parameter options.  Then add similar sentence
%% for the exonsBy example.
%% The \Rfunction{transcriptsBy} function allows grouping by gene, ...
...
specified by the \Robject{by} argument. These objects can be used by
functions like \Rfunction{findOverlaps} to contextualize alignments
from high-throughput sequencing.

%% 1. gene IDs depend on initial data source
%% 2. unique IDs not always available, use synthetic IDs instead

The identifiers used to label the groups depend upon the data source
used to create the \Rclass{TranscriptDb} object.  So the gene
identifiers will not always be Entrez Gene IDs, as shown in the
example above.  Furthermore, some data sources do not provide a unique
identifier for all features.  In this situation, the group label will
be a synthetic ID created by \Rpackage{GenomicFeatures} to keep the
relations between features consistent in the database.

%% FIXME: add example on how to map from internal ID to common ID
%% using the exons grouped by tx created above.

In most cases the grouped elements will be listed in the order that
they occur along the chromosome.  However, when exons or CDS are
grouped by transcript, they will instead be grouped according to their
position along the transcript itself.  This is important because
alternative splicing can mean that the order along the transcript can
be different from that along the chromosome.
