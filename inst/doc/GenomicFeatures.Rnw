...
\begin{enumerate}
\item Use \Rfunction{makeTranscriptDbFromUCSC} to download from UCSC Genome Bioinformatics.
\item Use \Rfunction{makeTranscriptDbFromBiomart} to download from BioMart.
\item Use a \Rclass{data.frame} containing transcript metadata with \Rfunction{makeTranscriptDb} to make a custom database. 
\end{enumerate}
...
return them as a single Granges object like this:
...
transcripts() function will allow you to do things like that too.
...
that would make the existance of our database schema impossible.  So
...
Then we need to used \Rfunction{findOverlaps} to lean which of our
data ranges dr, will overlap with the in exons that we have grouped by
transcripts.
...
\Rfunction{findOverlaps} call. And once we have subesetted out
annotations in this way, the lenth of this resulting GRangesList is
also the number of transcripts that overlap with our data.
...
represented as a GRanges object with the annotations stored in a
\Rclass{TranscriptDb} object.  Calling \Rfunction{findOverlaps} along
with the appropriate Rclass{GRanges} object, not only allows users to
quickly determine what has overlapped, but to also control what
criteria are used for determining whether an overlap has occurred.
This can be done by passing in an alternate type parameter to
findOverlaps.  In addition, because the basic accesors allow for the
users to retrieve data grouped in different ways, the user has control
over which parts of a transcript or gene are included in the overlap.
