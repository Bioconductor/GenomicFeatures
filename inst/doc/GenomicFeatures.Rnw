...
annotRD <- transcriptsByRanges(txdb, rd, columns=c("tx_id","tx_name",
                                           "gene_id", "exon_id"))
...
%annotRDEx <- exonsByRanges(txdb, rd)

%## Get the overlap between the annotations and your data
%OL <- findOverlaps(rd,annotRDEx, multiple=TRUE)

%## just get the data that you want
%edata <- annotRDEx[subjectHits(OL),]
...
the we have a collection of reads in a \Rclass{RangedData} object
...
transcriptsByRanges(txdb, rd, columns=c("tx_id","tx_name","gene_id"))


## So we will extend the rd object by a constant amount of 200 base pairs so
## that each read has an opportunity to overlap with something interesting.
rdExt <- rd
ranges(rdExt) <- ranges(rdExt)+100

## Then collapse the annotations
rdExt <- reduce(rdExt)

## And recover the annotations that match
annotRD <- transcriptsByRanges(txdb, rdExt, 
                               columns=c("tx_id","tx_name","gene_id"))

## Then find the number of unique matching gene IDs 
unique(unlist(annotRD$gene_id))
@
