...
%% TODO: clean up all examples and make it so that I don't use so many different values for txdb etc.
...
So from now on in this vignette, all answers should be limited to
chromosome 15.
...
%% TODO: call cdsBy 1st to get the ranges we really want.
<<extract>>=
