...
\subsection{Working with GRangesLists}

Sometimes you will want to consider ranged annotation data that is
grouped in a specific way.  There is a known relationship for example
between a gene and it stranscripts along with the exons that each
transcript contains.  And these relationships are preserved for you
along with their data in each TranscriptDb database.  In order to
allow easy access to this data and to its complex relational nature,
GenomicFeatures provides a special set of accessors.  These accessors
each return a GRangesList object.  The GRangesList object groups the
GRanges objects into a named list structure.  The grouping is in turn
controlled by the groupBy argument.  There is an accessor for
transcript ranges, exon ranges and cds ranges.
...
This GRangesList object can then be used with findOverlaps to only
search the spaces that are specified by each contained GRanges object,
which allows you to contextualize the search.
