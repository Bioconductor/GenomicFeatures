...
\Rfunction{discoverUCSCChrominfo} function to list them.  By default,
...
is the \Robject{DEFAULT_CIRC_SEQS} vector.  It contains strings that
...
<<discoverChromNames>>=
head(discoverUCSCChrominfo("hg19"))
@ 
...
of the \Robject{DEFAULT_CIRC_SEQS} vector.  And just like those UCSC
sources, there is also a helper function called
\Rfunction{discoverBiomartChrominfo} that can show what the different
