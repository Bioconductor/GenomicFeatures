...
\Rclass{GenomicFeatureList} object containing metadata for transcripts that
are overlapped by specified locations from a \Rclass{GenomicFeature} object.

<<transcriptsByRanges>>=
gf <- GenomicFeature(seqnames = rep("chr1",2),
                     ranges = IRanges(start=c(500,10500),end=c(10000,30000)),
                     strand = rep("-",2))
transcriptsByRanges(txdb, gf)
@


\subsection{Binding Transcript Data to a GenomicFeature Object}
...
##gf will represent the GenomicFeature object that you have (your data)
gf <- GenomicFeature(seqnames = "chr1",
...
annotGF <- transcriptsByRanges(txdb, gf, columns=c("tx_id","tx_name",
                                           "gene_id", "exon_id"))

## Get the overlap between the annotations and your data
OL <- findOverlaps(gf, annotGF, multiple=TRUE)

## just get the data that you want
tdata <- annotGF[subjectHits(OL),]
...
%annotGFEx <- exonsByRanges(txdb, gf)

%## Get the overlap between the annotations and your data
%OL <- findOverlaps(gf, annotGFEx, multiple=TRUE)

%## just get the data that you want
%edata <- annotGFEx[subjectHits(OL),]
...
the we have a collection of reads in a \Rclass{GenomicFeature} object
...
transcriptsByRanges(txdb, gf, columns=c("tx_id","tx_name","gene_id"))


## So we will extend the gf object by a constant amount of 200 base pairs so
## that each read has an opportunity to overlap with something interesting.
gfExt <- gf
ranges(gfExt) <- ranges(gfExt)+100

## Then collapse the annotations
gfExt <- reduce(gfExt)

## And recover the annotations that match
annotGF <- transcriptsByRanges(txdb, gfExt, 
                               columns=c("tx_id","tx_name","gene_id"))

## Then find the number of unique matching gene IDs 
unique(unlist(values(annotGF)$gene_id))
@
