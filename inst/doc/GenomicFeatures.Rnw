...
prespecified manner. The \Rfunction{fiveUTRsByTranscript} and
\Rfunction{threeUTRsByTranscript} functions are like this. These
functions retrieve the \Rclass{GRanges} objects for the relevant 5' or
3' UTR's grouped by transcript.
