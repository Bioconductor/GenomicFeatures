test_transcripts <- function()
{
    ...
    want <- GenomicFeature(seqnames = c("chr1", "chr5"),
                           ranges = wantRanges,
                           strand  = strand(rep("-", 2)),
                           tx_id   = c(3L, 15L),
...
test_exons <- function()
{
    ...
    checkIdentical(exons(txdb, list("exon_id" = 1)),
                   split(GenomicFeature(seqnames = "chr1",
    ...
    want <- split(GenomicFeature(seqnames = c(rep("chr1", 4), rep("chr5", 3)),
                                 ranges = wantRanges,
                                 strand = strand(rep("-", 7)),
...
test_cds <- function()
{
    ...
    checkIdentical(cds(txdb, list("cds_id" = 91)),
                   GenomicFeature(seqnames = "chr10",
                                  ranges = IRanges(start=82997, end=84054),
                                  strand = strand("-"),
                                  cds_id = 91L))

    want <- GenomicFeature(seqnames = c("chr5", "chr5"),
